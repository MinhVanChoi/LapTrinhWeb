...
public class UserDAOImplement implements IUserDAO {
	...
	public List<UserModel> findAll() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new UserModel(rs.getInt("user_id"), rs.getString("user_name"), 
                		rs.getString("user_password"),rs.getString("user_email"), 
                		rs.getString("user_fullname"),rs.getString("user_avatar"),rs.getInt("user_role_id"),
                		rs.getString("user_phone"), rs.getDate("user_datecreate")));
            }
            return list;
        } catch (Exception e) {
            e.printStackTrace();
	...
	public UserModel findById(int id) {
		 String query = "SELECT * FROM users where user_id = ? ";
	        ...
	        try {
	            ...
	            rs = ps.executeQuery();
	            while (rs.next()) {
	                user = new UserModel(rs.getInt("user_id"), rs.getString("user_name"), 
	                		rs.getString("user_password"),rs.getString("user_email"), 
	                		rs.getString("user_fullname"),rs.getString("user_avatar"),rs.getInt("user_role_id"),
	                		rs.getString("user_phone"), rs.getDate("user_datecreate"));
	            }
	...
	public void insert(UserModel user) {
		 List<UserModel> users = findAll();
	        String query = "insert into users (user_name, user_password, user_email,"
	        		+ "user_fullname,user_avatar, user_role_id,user_phone,user_datecreate)"
	        		+ " values (?,?,?,?,?,?,?,?)";
